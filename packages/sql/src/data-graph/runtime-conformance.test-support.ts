...
export const dataGraphRuntimeConformance = (
  ...
  describe(`${name} data graph runtime conformance`, () => {
    ...
    it.skipIf(capabilities.upsert === false)(
      'implements ignore and merge upsert strategies',
      async () =>
        withHarness(async ({ runtime }) => {
          const { BookWithChapters } = conformanceGraph;
    ...
    it.skipIf(capabilities.upsert === false)('rejects malformed upsert commands', async () =>
      withHarness(async ({ runtime }) => {
        const { BookWithChapters } = conformanceGraph;
