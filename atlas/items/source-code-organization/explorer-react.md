...
9. the entity browser renders a package-owned Data tab when `@ontahi/react/graph` has a reflected entity data reader; its active Entity rows and ephemeral query controls live in one compact movable and collapsible collection-view node whose header owns Entity selection, while collection identity remains distinct from portable instance identity; BookOps currently supplies a temporary reader adapter around its existing server action,
