...
- d293ff1: Bind Ref operation shortcuts to the matching schema-native `ref` or `existingRef` input participant,
...
- d293ff1: Validate schema-native Operation references against declared locators, including composite identities and alternate locators, without applying persisted foreign-key restrictions. Keep single-field storage identity checks unchanged for `field.ref`.
