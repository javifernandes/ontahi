...
export const createSqlDerivedFieldCompiler = (dialect: SqlDialect) => {
  const quote = dialect.quoteIdentifier;

  const compileRelationCount = (
    entity: AnyEntityDefinition,
    relationName: string,
    rootMapping: SqlEntityMapping,
  ) => {
    ...
    if (mapping.type === 'many-to-many') {
      ...
      return (
        `(SELECT ${dialect.countExpression} FROM ${quote(mapping.throughTable)} AS ${quote(alias)}` +
    ...
    return (
      `(SELECT ${dialect.countExpression} FROM ${quote(targetTable)} AS ${quote(alias)}` +
  ...
  const compileSqlDerivedField = (
    ...
    if (semanticMapping.tableName !== mapping.table) {
      throw new Error(`SQL mapping does not match derived Field Entity ${entity.name}.`);
    }
    return compileExpression(program.expression, entity, mapping);
  };

  return compileSqlDerivedField;
};
