...
the same checked-out connection. A full destination or a capacity compare-and-set mismatch returns
a domain failure and rolls the complete transition back. A stale `previousCourse` is translated
from the portable Relationship Command outcome into `student_course_changed`, without leaking a
provider error through the Domain Operation contract.
