...
// Build-time compiler for the portable Ontahi Model Expression subset.
...
const expressionValue = (semantic, expression, model = {}) => ({
  semantic,
  expression,
  ...model,
});
...
const compileCall = (call, context) => {
  ...
  if (receiver.entityName && right.entityName && receiver.entityName !== right.entityName) {
    return failAt(
      context,
      call.arguments[0],
      'model_expression_ref_target_mismatch',
      `Cannot compare ${receiver.entityName} and ${right.entityName} Ref identities.`,
    );
  }
...
export const compileModelExpressionCallback = (
  callback,
  { sourceFile, sourcePath = sourceFile.fileName, symbols },
) => {
  const initializer = unwrapExpression(callback);
...
export const analyzeModelExpressionSource = (
  ...
  return compileModelExpressionCallback(declaration.initializer, {
    sourceFile,
    sourcePath,
    symbols,
  });
