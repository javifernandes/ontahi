...
Ontahi for Developers is the programming-facing book originally created in the Ontahi Library.
Its canonical source now lives beside the framework at
[`docs/developers`](../../../../docs/developers/README.md), while _Living Systems_ remains in the
separate `ontahi-library` repository. The moved source records the original repository, path, and
commit, and the former location keeps only a relocation notice rather than a synchronized copy.

The book teaches the framework directly through its canonical declarations, runtime boundaries,
...
The independent Todo Express application is the simple executable spine, the headless Classroom
example proves richer Relation lifecycle, and BookOps is the production pressure test. The
first-edition inventory classified the actual exported and used surface as
...
The first edition is complete and publication-verified: 28 chapters in five parts move from a
...
Before each release, accumulated Changesets, completed Plans, durable Atlas changes, packed
artifacts, Todo, and Classroom are reconciled against this canonical source. The Relations
lifecycle edition teaches semantic reflected endpoints, conditional and constrained Relationship
